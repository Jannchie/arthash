//! wasm-bindgen wrapper around `arthash` (Rust core). Exposes the same three
...
//! `CodecOptions` struct via `serde-wasm-bindgen` and rebuild a `arthash::Codec`
...
/// Encode an RGB buffer (row-major, 3 bytes per pixel) into a arthash byte
/// string.
