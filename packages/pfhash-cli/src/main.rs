//! `pfhash` CLI — encode images to placeholder hashes; decode hashes back to
...
/// Shape-mode encoder thumbnail long-edge (mirrors `pfhash::shape::THUMB`).
const SHAPE_TARGET: u32 = 48;

#[derive(Parser)]
#[command(name = "pfhash", version, about = "Encode images to placeholder hashes and back.")]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
}
...
#[derive(Subcommand)]
enum Cmd {
    /// Encode an image file (PNG/JPEG/WebP) into a pfhash.
    Encode {
        /// Input image path.
        input: PathBuf,
    ...
    /// Decode a pfhash into a preview PNG.
    Decode {
    ...
    /// Render a pfhash as an SVG (CIRCLE / TRIANGLE only).
    Svg {
        /// Path to hash bytes. `-` ⇒ stdin.
        input: PathBuf,
        /// Output path. `-` or omitted ⇒ stdout.
