...
Rust SDK for pfhash — the canonical implementation of the byte-format
contract in [`docs/SPEC.md`](../../docs/SPEC.md). The Python SDK
(`packages/pfhash-py`) is a PyO3 wrapper around this crate; the TypeScript
SDK (`packages/pfhash-ts`) is a wasm-bindgen wrapper. Four shape modes share
...
```rust
use pfhash::{Codec, ShapeType, encode_rgb, decode, EncodeOptions, DecodeOptions};
...
means resize to `pfhash::shape::THUMB = 48` long-edge before calling. For
