...
//! crate (see `python/pfhash/`). The wrapper owns the user-facing
...
use numpy::PyArray1;
use pfhash::shape::options::Strategy;
...
fn to_svg(
    ...
    py.detach(|| rs_to_svg(hash, &codec, opts))
        .map_err(|e| match e {
            pfhash::SvgError::UnsupportedShape(_) => PyNotImplementedError::new_err(e.to_string()),
        })
}
